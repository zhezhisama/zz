...
class TagLibHtml extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'list'=>array('attr'=>'id,pk,style,action,actionlist,show,datasource,checkbox,$selected','close'=>0),
    ...
    public function _list($attr)
    {
        ...
        foreach($fields as $field) {//显示指定的字段
            ...
            if($sort) {
                $parseStr .= '<a href="javascript:sortBy(\''.$property[0].'\',\'{$sort}\',\''.ACTION_NAME.'\')" title="按照'.$showname[2].'{$sortType} ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ></eq></a></th>';
            }else{
                $parseStr .= $showname[0].'</th>';
            }
        ...
        if(!empty($action)) {//如果指定显示操作功能列
            $parseStr .= '<th align="center">操作</th>';
        }
        ...
        if(!empty($checkbox)) {//如果需要显示checkbox 则在每行开头显示checkbox  
            ...
            if(!empty($selected)) {
                $parseStr .= '<?php if(!empty($'.$selected.') && ($'.$selected.' == $'.$name.'['.$pk.'] || in_array($'.$name.'['.$pk.'],$'.$selected.'))) { ?>';
                $parseStr .= '<td><input type="checkbox" name="key" value="{$'.$name.'.'.$pk.'}" checked="true"></td>';
                $parseStr .= '//<?php }else { ?>';
            ...
            if(!empty($field[2])) {
                ...
                if(count($href)>1) {
                    ...
                    if(count($array)>1) {
                        ...
                        //$parseStr .= '<a href="javascript:'.$href[0].'(\'{$'.$name.'.'.$href[1].'|addslashes}\')">';
                        $parseStr .= '<a href="__URL__/'.$href[0].'/id/{$'.$name.'.'.$href[1].'|addslashes}">';
                    }
                }else {
                    //如果没有指定默认传编号值
                    if($field[2]=="child")
                    {
                        $parseStr .= '<a href="__URL__/index/pid/{$'.$name.'.'.$pk.'|addslashes}">';
                    }
                    else
                    {
                        //$parseStr .= '<a href="javascript:'.$field[2].'(\'{$'.$name.'.'.$pk.'|addslashes}\')">';
                        $parseStr .= '<a href="__URL__/'.$field[2].'/id/{$'.$name.'.'.$pk.'|addslashes}">';
                    }
                }
            }
            if(strpos($field[0],'^')) {
        ...
        if(!empty($action)) {//显示功能操作
            if(!empty($actionlist[0])) {//显示指定的功能项
                $parseStr .= '<td align="center">';
                foreach($actionlist as $val) {
                    if(strpos($val,':')) {
                        $a = explode(':',$val);
                        if(count($a)>2) {
                            //$parseStr .= '<a href="javascript:'.$a[0].'(\'{$'.$name.'.'.$a[2].'}\')">'.$a[1].'</a>&nbsp;&nbsp;&nbsp;&nbsp;';
                            $parseStr .= '<a style="margin:0 5px;" href="__URL__/'.$a[0].'/id/{$'.$name.'.'.$a[2].'}">'.$a[1].'</a>';
                        }else {
                            //$parseStr .= '<a href="javascript:'.$a[0].'(\'{$'.$name.'.'.$pk.'}\')">'.$a[1].'</a>&nbsp;&nbsp;&nbsp;&nbsp;';
                            $parseStr .= '<a style="margin:0 5px;" href="__URL__/'.$a[0].'/id/{$'.$name.'.'.$pk.'}">'.$a[1].'</a>';
                        }
                    }else{
                        $array = explode('|',$val);
                        ...
                        if(count($array)>2) {
                            if($array[1]=="app"){
                                $parseStr.= '<a style="margin:0 5px;" href="__URL__/'.$array[1].'/groupId/{$'.$name.'.'.$array[0].'}">'.$array[2].'</a>';
                            }  else {
                                //$parseStr.= '&nbsp;&nbsp;<a href="javascript:'.$array[1].'(\'{$'.$name.'.'.$array[0].'}\')">'.$array[2].'</a>&nbsp;&nbsp;';
                                $parseStr.= '<a style="margin:0 5px;" href="__URL__/'.$array[1].'/id/{$'.$name.'.'.$array[0].'}">'.$array[2].'</a>';
                            }
