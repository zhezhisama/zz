...
if($uid){
	...
	if(!$weixin) exit("<script>alert('请先上传二维码');location.href='list.php'</script>");	//是否有这个记
}
	$now = time();
	$top_time = mysql_query("SELECT min(top_time) FROM `weixin` where `top` = 3 and `top_time` >= $now ");
...
if($_POST){
	...
	if($now < $weixin['top_time']){
		exit("<script>alert('现在你还存在你置顶状态，请等待置顶时间结束。');location.href='refresh.php'</script>");
	}
	
	if($price < $need_price){
		exit("<script>alert('对不起您没有足够的积分，请充值');location.href='refresh.php'</script>");
	}
	
	if($count['num'] >= 3 && $top == 3){
		exit("<script>alert('对不起，现在没有土豪置顶名额');location.href='refresh.php'</script>");
	}
	...
	if(mysql_affected_rows() == 1){
		...
		qy_close();
		echo "<script type='text/javascript'>alert('置顶成功!');location.href='list.php';</script>";
		exit;
	}else{
		qy_close();
		qy_alert_back('置顶失败!');
	...
	<div id="member-container" style="display: block;">
      ...
      <div id="tx-container" style="display: block !important;  padding-top:5px;" >  
        <section class="order">
            ...
            <form name="reform" id="reform" method="post" action="refresh.php" >
