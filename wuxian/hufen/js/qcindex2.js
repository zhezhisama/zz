...
$(function() {
   ...
   // $("a.upcode").click(function() {
    ...
    $("a.refresh").click(function() {
        if (/*vip &&*/ fansid) {
           /* if (upnum >= 10 && vip == 2) {
                alert("亲爱的,您是试用VIP，每日只能置顶10次（刚发布算1次）！\n\n购买包月VIP，即可不受置顶次数的限制！");
                return false
            }
            var timestamp = Math.round(new Date() / 1000);
            if (storage.getItem("refresh")) {
                if (timestamp - mytime < 600) {
                    alert("距您上上次刷新不到10分钟,请您休息会,稍后再刷新!\n\n还需等待：" + (600 - (timestamp - mytime)) + "秒！");
                    return false
                }
            }*/
          /*  if (vip != 1) {
            ...
            $.get("up2.php?a=upfans&r=" + timestamp, {},
            function(data) {
                var info = eval(data);
                if (info.sta == "ok") {
                    //storage.setItem("refresh", timestamp);
                    alert("刷新成功！");
                    window.location.href = "listqun.php?r=" + Math.round(new Date())
                } else {
                    if (info.sta == "time") {
        ...
        if ($(document).scrollTop() + 100 >= $(document).height() - $(window).height()) {
            if (stop == true) {
                ...
                setTimeout(function() {
                    ...
                    $.get("fans2.php?search=" + search + "&province=" + province + "&city=" + city + "&sex="+ sex +"&page=" + page + "&utime=" + utime + "&r=" + Math.random(), {},
                    function(data) {
                        var info = eval(data);
                        if (info.sta == "err") {
                            stop = true;
