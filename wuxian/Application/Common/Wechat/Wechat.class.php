...
class Wechat
{
	...
	*/
	 ...
	 * 
	 * */
	public function tonews($touser,$newsData=array())
	{
	    $FuncFlag = $this->_funcflag ? 1 : 0;
	    $count = count($newsData);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			F($authname,$this->access_token);//update by php@妖孽
			return $this->access_token;
		}
		return false;
	}
	...
	public function createPackage($appid,$mch_id,$nonce_str,$out_trade_no,$body,$total_fee,$notify_url,$spbill_create_ip,$fee_type=1,$bank_type="WX",$input_charset="UTF-8",$time_start="",$time_expire="",$transport_fee="",$product_fee="",$goods_tag="",$attach=""){
			$arrdata = array("mch_id" => $mch_id,"bank_type" => $bank_type, "body" => $body, "partner" => $this->partnerid, "out_trade_no" => $out_trade_no, "total_fee" => $total_fee, "fee_type" => $fee_type, "notify_url" => $notify_url, "spbill_create_ip" => $spbill_create_ip, "input_charset" => $input_charset,'issubscribe'=>1);
	...
	public function checkOrderSignature($orderxml=''){
		...
		$arrdata = array('appid'=>$orderxml['AppId'],'appkey'=>$this->paysignkey,'timestamp'=>$orderxml['TimeStamp'],'noncestr'=>$orderxml['NonceStr'],'openid'=>$orderxml['OpenId'],'issubscribe'=>1);
		$paySign = $this->getSignature($arrdata);
