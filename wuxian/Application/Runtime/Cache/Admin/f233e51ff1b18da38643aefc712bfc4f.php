...
$(function(){
	$("#goto_button").click(function(){
		if($("#goto").val())
			location='index.php?g=Admin&m=Order&a=index&p=' + $("#goto").val();
	})
})
</script>
								...
								<div class="table-responsive">
									<table id="sample-table-1"
										...
										<?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$result): $mod = ($i % 2 );++$i;?><tr>
												...
												<td><?php if($result['pay_status'] == 1): ?>已支付<?php else: ?>未支付<?php endif; ?></td>
												<td>
													<?php if($result['order_status'] == 0): ?>未发货<?php endif; ?>
													<?php if($result['order_status'] == 1): ?>已发货<?php endif; ?>
													<?php if($result['order_status'] == 2): ?>已收货<?php endif; ?>
													<?php if($result['order_status'] == 3): ?>已完成<?php endif; ?>
													<?php if($result['order_status'] == 4): ?>已退货<?php endif; ?>
												</td>
												...
												<td>
													订单号：<input type='text' name='num' id='num_<?php echo ($result["id"]); ?>' value='<?php $json = $result[order_info];$data = json_decode($json,true);echo $data[num] ?>'><br/>
												...
												<td>
													...
													<!--a href="<?php echo U('Admin/Order/payComplete',array('id'=>$result['id']));?>" class="btn btn-white btn-sm">已支付</a-->
