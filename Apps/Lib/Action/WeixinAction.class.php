...
class WeixinAction extends CommonAction{
    public function index() {
        ...
        $list   = $Form->Table($prefix.'weixin w')->join('(select * from '.$prefix.'tuijian WHERE recommendid=3 AND (timelimit=0 OR endtime>=unix_timestamp(CURDATE())) AND status=1) t ON w.id=t.wxid')->where($map)->field('w.id,w.pubaccount,w.logo,w.weblogo,w.qrcode,w.webqrcode,w.content,w.tupian,w.tag,w.xingji,w.hits,w.status,t.recommendid,w.create_time,IFNULL(t.create_time,9369311152) as tjtime')->limit($Page->firstRow. ',' . $Page->listRows)->order($order['order'])->select();
